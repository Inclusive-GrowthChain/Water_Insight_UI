...
export async function createStake(data, onSuccess, onError) {
  try {
    await enableMetaMask();
    await myContract.myContract.methods
      .Project_StakeMoney(Number(data.stakeAmount), data.projectId)
      .send({ from: window?.ethereum?.selectedAddress });
...
export async function vote(data, onSuccess) {
  try {
    ...
    await myContract.myContract.methods
      .Project_CastVote(data.type, data.id)
      .send({ from: window?.ethereum?.selectedAddress });
...
export async function closeProject(projId, onSuccess, onError) {
  try {
    ...
    const project = await myContract.myContract.methods
      .closeVoting(projId)
      .send({ from: window?.ethereum?.selectedAddress });
...
export async function unstakeAmt({ id, amt }) {
  try {
    ...
    const unstakeProject = await myContract.methods
      .Project_UnStakeMoney(amt, id)
      .send({ from: window?.ethereum?.selectedAddress });
